...
- Les produits génériques ayant des catégories, des attributs, des options, des variantes, des produits groupés, etc.
- Créer/gérer des dossiers/lots (produits, volumes…)
- Gérer les contrats
- Valider des produits des fournisseurs

**Un back office fournisseur,** où les fournisseurs peuvent essentiellement gérer les produits associés aux contrats auxquels ils sont rattachés. Ils peuvent y spécifier pour chaque produit :
...
L’équipe chargée de ce projet est relativement petite, constituée d’une cheffe de projet, de deux alternants et d’un ou deux autres développeurs selon la période.
...
Je vais maintenant détailler certaines fonctionnalités clé que j’ai mises en place sur le projet.
...
Pour réaliser cette fonctionnalité, la première étape a été de lire attentivement les différentes user stories sur Jira, d'analyser les maquettes et de discuter avec le chef de projet pour mettre au clair les zones d’ombre. Une fois que j’avais bien compris le processus et le besoin métier, j’ai créé un diagramme de classe pour représenter les entités que j’allais devoir créer.
...
Une fois la structure en place, la prochaine tâche a été de mettre en place le formulaire de création de thème, la liste de thèmes, la liste paginée des discussions de chaque thème, ainsi que le formulaire d’ajout de discussion. Pour cela, j’ai ajouté de nouveaux droits à la plateforme, j’ai créé le contrôleur, les vues Twig et les formulaires Symfony nécessaires. J’ai ensuite créé la page la plus importante du forum, la page discussion. Cette page contient la liste des messages par ordre chronologique et paginée. Elle permet aux opérateurs de supprimer ou modifier des messages, ainsi qu’à leurs créateurs respectifs. Elle permet aussi de naviguer page par page et d’ajouter un nouveau message. Le champ qui permet d’ajouter un nouveau message est un champ de texte riche. Sur Mon Portail Achats, nous utilisons CKEditor, une librairie JavaScript permettant de créer des champs de texte riches avec une multitude de fonctionnalités comme la mise en page, l’ajout de documents, de photos ou d’emoji, et de pouvoir ensuite afficher tout ça au format HTML. L’ajout d’un message possède aussi un système de notification permettant de notifier tout ou partie des utilisateurs de la plateforme.

J’ai donc dû par la même occasion créer un système de notification permettant d’afficher une cloche de notification aux utilisateurs qui change de couleur quand un nouveau message apparaît et qui permet d’afficher les notifications sur une fenêtre flottante quand on clique dessus, et les marque comme lues une fois la fenêtre ouverte.
...
Ce projet est quand même dans l’ensemble une réussite. Nous avons pu apporter des solutions à chacune des problématiques rencontrées, la vélocité a été bonne, et le client a rapidement pris confiance. La cheffe de projet s’est déplacée un peu partout en France pour former les utilisateurs suite à la mise en production, et leurs retours quant à ce nouveau portail d’achats ont été très positifs. Le client souhaite continuer à travailler avec nous pour faire évoluer le portail et prépare déjà le backlog pour une V2.
